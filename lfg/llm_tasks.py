...
def prepare_prompt_summary(text):
    text = text.replace("\n", " ")
    text = text + '\n TL;DR: '
    text = text.strip()
...
def prepare_prompt_classification(text, positive_examples, negative_examples=None):
    # Format positive examples
    positive_text = "\n".join(f"Positive Example: {example}" for example in positive_examples)
    
    # Format negative examples if provided
    if negative_examples:
        negative_text = "\n".join(f"Negative Example: {example}" for example in negative_examples)
        examples_text = f"{positive_text}\n{negative_text}"
    else:
        examples_text = positive_text
    ...
    prompt = f"Examples:\n{examples_text}\n\nGiven Text: {text}\n\nIs the given text more like the positive examples? Answer with 'Yes' or 'No'."
...
def main(args):
    ...
    try:
        datapoints = [
            prepare_prompt_summary(next(long_texts)) for i in range(args.max_ite)
        ]
